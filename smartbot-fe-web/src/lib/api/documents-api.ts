...
/** Get signed URL for markdown content. */
export function getDocumentMarkdownUrl(kbId: string, docId: string) {
  return apiGet<{ url: string }>(`${base(kbId)}/${docId}/markdown`)
}

/** Get signed URL for document image. */
export function getDocumentImageUrl(kbId: string, docId: string, filename: string) {
  return apiGet<{ url: string }>(`${base(kbId)}/${docId}/images/${encodeURIComponent(filename)}`)
}
