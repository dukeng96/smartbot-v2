...
export function useDocumentMarkdownUrl(kbId: string, docId: string, enabled: boolean) {
  return useQuery({
    queryKey: [...KEYS.detail(kbId, docId), "markdownUrl"],
    queryFn: () => getDocumentMarkdownUrl(kbId, docId),
    enabled: enabled && !!kbId && !!docId,
    staleTime: 30 * 60 * 1000,
  })
}

export function useDocumentMarkdownContent(url: string | undefined) {
  return useQuery({
    queryKey: ["markdownContent", url],
    queryFn: async () => {
      if (!url) return ""
