...
import type { NodeData, StateUpdate, FlowData, NodeDefinition, NodeTrace, ConditionRule, Intent } from "@/lib/types/flow"
import { createNode, buildNodeData } from "../utils/node-factory"
import { encodeHandleId } from "../utils/handle-id"
    ...
    updateNodeConfig: (id, key, value) =>
      set((state) => {
        ...
        if (node) {
          ...
          // Regenerate outputAnchors for dynamic output nodes when conditions/intents change
          const dynamicOutputs = node.data.definition.dynamicOutputs
          if (dynamicOutputs && (key === "conditions" || key === "intents")) {
            const items = (value as Array<ConditionRule | Intent>) || []
            const newOutputAnchors = items.map((_, index) => ({
              id: encodeHandleId(id, "out", String(index), "any"),
              name: String(index),
              type: "any",
            }))
            node.data.outputAnchors = newOutputAnchors

            // Remove orphaned edges (edges pointing to removed handles)
            const validHandleIds = new Set(newOutputAnchors.map((a) => a.id))
            state.edges = state.edges.filter((e) => {
              if (e.source !== id) return true
              // Keep edge if its sourceHandle is still valid
              return e.sourceHandle ? validHandleIds.has(e.sourceHandle) : true
            })
          }
