...
export const GenericNode = memo(function GenericNode({
  ...
  // Compute min-height so all output handles fit inside the node
  const handleCount = Math.max(inputAnchors.length, outputAnchors.length)
  // Header ~44px + handle spacing (22px each) + padding
  const minHeight = handleCount > 1 ? 44 + handleCount * 22 + 16 : undefined
  ...
  return (
    ...
    >
      ...
      {previewEntries.length > 0 && (
        <div className="px-4 py-3 space-y-1 text-[12px] text-muted-foreground">
